...
data= pd.read_csv("resultupto400.csv")
X_h = data[:][['X','Y','Q_cms']]
y_h = data[:]['Depth']
X_wd=data[:][['X','Y','Q_cms']]
y_wd = data[:]['IBC']
...
X_h600 = data600[:][['X','Y','Q_cms']]
y_h600 = data600[:]['Depth']
X_wd600=data600[:][['X','Y','Q_cms']]
y_wd600 = data600[:]['IBC']
X_z600= data600[:][['X','Y']]
...
#      
#regressor_h = RandomForestRegressor(n_estimators=25, random_state=120,n_jobs=-1)
#
...
#
#print('results for Q=300\n')
#
#print('Mean Absolute Error regression depth for q=300:', metrics.mean_absolute_error(y_h600, y_pred_h600))
...
clf= RandomForestClassifier(n_estimators=10, random_state=5,n_jobs=-1)
...
print('\n')
print('Mean Absolute Error classify wet dry Q=300:', metrics.mean_absolute_error(y_wd600, y_pred_wd600))
